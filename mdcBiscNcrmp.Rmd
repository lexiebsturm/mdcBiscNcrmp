...
pacman::p_load("paletteer", "patchwork", "tidyverse", "vegan",  "dplyr", "ncrmp.benthics.analysis", "ggplot2", "sf", "maps", "ggmap", "ggspatial", "tigris", "glue", "ggpubr", "multcomp", "ggbreak", "cowplot")
...
geom_sf(data = south_florida_counties, fill = NA, color = "black") +
  ...
  labs(title = "NCRMP LPI Surveys in Miami-Dade County & Biscayne National Park from 2014-2022",
       x = "Longitude",
       y = "Latitude") +
...
ggsave(
  filename = "./figs/mdcBiscBenthicMap.png",    # Change the filename and extension as needed
  plot = mdcBiscBenthicMap,              # The plot object you want to save
  width = 12,                            # Width of the output file in inches
  height = 8,                            # Height of the output file in inches
...
geom_sf(data = south_florida_counties, fill = NA, color = "black") +
  ...
  geom_text(data = centroids, aes(x = X, y = Y, label = NAME), size = 3, color = "blue") +
  geom_sf(data = biscayne_park, fill = NA, color = "green", size = 1.5, aes(linetype = "Biscayne National Park")) +  # Outline of Biscayne National Park
  labs(title = "Coral Demographic Surveys in Miami-Dade County & Biscayne National Park from 2002-2022",
       x = "Longitude",
       y = "Latitude",
       color = "Survey Type") +
   coord_sf(xlim = c(-84, -79), ylim = c(24, 27), expand = FALSE) +  # Set the bounding box for the map
...
ggsave(
  filename = "./figs/mdcBiscDemoSurveysMap.png",    # Change the filename and extension as needed
  plot = mdcBiscDemoSurveysMap,              # The plot object you want to save
  width = 12,                            # Width of the output file in inches
  height = 8,                            # Height of the output file in inches
  dpi = 300                              # Resolution in dots per inch
)
...
write.csv(mdcBiscBenthicSurveysTotal, "./data/mdcBiscBenthicSurveyNumbers.csv")
...
write.csv(mdcBiscDemoSurveyCounts, "./data/mdcBiscDemoSurveyCountsNumbers.csv")
```

##Frequency of Incidence (FOI)
###FOI Bleaching
...
write.csv(mdcBiscBleachingSummary, "./data/mdcBiscBleachingSummary.csv")
```

```{r, FOI bleaching chart}
# Your data preparation code (as given)
...
# Create the stacked bar chart without title
mdcBiscBleachingChart <- ggplot(mdcBiscBleachingLong, aes(x = YEAR, y = Count, fill = BleachingStatus)) +
   geom_bar(stat='identity', position = position_stack(reverse = TRUE)) +
  labs(x = "Year",
       y = "Number of Surveys",
       fill = "Bleaching Status") +  # Set legend title
  scale_fill_manual(values = c("surveys_with_bleaching" = "darkred", 
                                "surveys_without_bleaching" = "lightgray"),
                    labels = c("Bleaching Present", "Bleaching Absent")) +
  theme_minimal() +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1))  # Remove legend for the first plot
...
mdcBiscBleachingProportions <- mdcBiscDemoBleachingSurveys %>%
  ...
  dplyr::select(YEAR, proportion_with_bleaching, proportion_without_bleaching) %>%
  mutate(YEAR = as.character(YEAR))
...
# Create the proportional bar chart without title
mdcBiscBleachingProportionsChart <- ggplot(mdcBiscBleachingLongProportions, aes(x = YEAR, y = Proportion, fill = BleachingStatus)) +
  ...
  labs(x = "Year",
       ...
       fill = "Bleaching Status") +  # Set legend title
  scale_fill_manual(values = c("proportion_with_bleaching" = "darkred", 
                                "proportion_without_bleaching" = "lightgray"),
                    labels = c("Bleaching Present", "Bleaching Absent")) +
  theme_minimal() +
  theme(legend.position = "bottom",  # Place legend at the bottom
        legend.box = "vertical",
        legend.direction = "vertical",
        axis.text.x = element_text(angle = 45, hjust = 1))  # Stack legend items vertically

# Combine the plots using patchwork and ensure a single legend
combinedBleachingplot <- mdcBiscBleachingChart | mdcBiscBleachingProportionsChart +
  plot_layout(guides = "collect")  # Collect and combine legends

# Save the combined plot
ggsave("./figs/combinedBleachingplot.png", plot = combinedBleachingplot, width = 12, height = 6, units = "in")
...
mdcBiscDiseaseProportions <- mdcBiscDiseaseSummary_no_total %>%
  ...
  dplyr::select(YEAR, proportion_disease_present, proportion_disease_absent) %>%
...
# Create the stacked bar chart for total surveys without legend and title
mdcBiscDiseaseTotalChart <- ggplot(mdcBiscDiseaseLong, aes(x = YEAR, y = Count, fill = DiseaseStatus)) +
  geom_bar(stat = "identity", position = position_stack(reverse = TRUE)) +
  labs(x = "Year",
       y = "Number of Surveys",
       fill = "Disease Status") +  # Set legend title
  scale_fill_manual(values = c("disease_present" = "yellow", 
...
# Create the stacked bar chart for proportional data with updated legend and no title
mdcBiscDiseaseProportionChart <- ggplot(mdcBiscDiseaseProportions, aes(x = YEAR, y = Proportion, fill = DiseaseStatus)) +
  geom_bar(stat = "identity", position = position_fill(reverse = TRUE)) +
  labs(x = "Year",
       y = "Proportion of Surveys",
       fill = "Disease Status") +  # Set legend title
  scale_fill_manual(values = c("proportion_disease_present" = "yellow", 
                                "proportion_disease_absent" = "lightgray"), 
                    labels = c("Disease Present", "Disease Absent")) +
  theme_minimal() +
  theme(legend.position = "bottom",  # Place legend at the bottom
...
# Combine the plots using patchwork and ensure a single legend
combinedDiseaseplot <- mdcBiscDiseaseTotalChart | mdcBiscDiseaseProportionChart +
  plot_layout(guides = "collect")  # Collect and combine legends

# Save the combined plot
ggsave("./figs/combinedDiseaseplot.png", plot = combinedDiseaseplot, width = 12, height = 6, units = "in")
...
# Define color palette with 8 colors
colPal <- paletteer::paletteer_d("NineteenEightyR::youngturqs", n = 8)

# Ensure your data is prepared correctly
mdcBiscDemo <- allDemo %>%
  mutate(SurveyCode = paste(YEAR, MONTH, DAY, PRIMARY_SAMPLE_UNIT, sep = "-")) %>%
  mutate(MilSortCode = paste(YEAR, MONTH, DAY, PRIMARY_SAMPLE_UNIT, STATION_NR, sep = "-")) %>%
  filter(SUB_REGION_NAME %in% c("Biscayne", "Broward-Miami")) %>%
  ...
  mutate(SPECIES_CD = if_else(SPECIES_CD == "ORB ANCX", "ORB ANNU", SPECIES_CD)) %>%  # Correcting the species code
  mutate(SPECIES_NAME = if_else(SPECIES_NAME == "Orbicella annularis species complex", "Orbicella annularis", SPECIES_NAME)) %>%  # Correcting the species name
  arrange(YEAR, MONTH, DAY, REGION, MilSortCode)

# Define the species of interest
listedSpp <- c("Acropora cervicornis", "Acropora palmata", "Dendrogyra cylindrus", 
               "Mycetophyllia ferox", "Orbicella annularis", "Orbicella faveolata", 
               "Orbicella franksi", "Orbicella spp")

# Filter and summarize data
esaOnlyDemo <- mdcBiscDemo %>%
  filter(SPECIES_NAME %in% listedSpp, N >= 1) %>%
  group_by(SPECIES_NAME, YEAR) %>%
  summarise(count = n(), .groups = 'drop') %>%
  pivot_wider(names_from = SPECIES_NAME, values_from = count, values_fill = list(count = 0))

# Convert to long format for plotting
esaSppFOI_long <- esaOnlyDemo %>%
  pivot_longer(cols = -YEAR, names_to = "Species", values_to = "Count") %>%
  mutate(Species = factor(Species, levels = listedSpp))  # Ensure correct order

# Function to create plot with italicized species names in titles
create_plot <- function(species_name, color) {
  ggplot(esaSppFOI_long %>% filter(Species == species_name), aes(x = YEAR, y = Count, group = Species, color = Species)) +
    geom_line(size = 1) +
    geom_point(size = 2) +
    labs(title = glue("{species_name}"),  # Use glue for dynamic titles
         x = "Year",
         y = "Count of Surveys") +
    scale_color_manual(values = color) +  # Use the assigned color
    theme_minimal() +
    theme(legend.position = "none",  # Remove legend
          axis.text.x = element_text(angle = 45, hjust = 1),  # Rotate x-axis labels
          plot.title = element_text(face = "italic"))  # Italicize title
}

# Generate individual plots with rotated colors
plot_list <- Map(function(species_name, color) {
  create_plot(species_name, color)
}, listedSpp, colPal)  # Map each species to a color

# Combine all plots in a grid
combined_plot <- wrap_plots(plot_list, ncol = 2)  # Adjust ncol for desired grid layout

# Save the combined plot
ggsave("./figs/combinedSpeciesPresenceAbsence.png", plot = combined_plot, width = 12, height = 12, units = "in")
```

##Disease & Bleaching Prevalence
```{r, disease prev}
#drmScreamDis=DRM_SCREAM_FL_1999_13_dis_prev_site
#Reading this package in wasn't working :(

sefcriDisBle=NCRMP_DRM_SEFCRI_2014_22_dis_ble_prev_site
flkDisBle=NCRMP_DRM_FLK_2014_22_dis_ble_prev_site
flDisBle <- bind_rows(
  sefcriDisBle,
  flkDisBle
)

mdcBiscDisBle=flDisBle %>%
filter(SUB_REGION_NAME %in% c("Biscayne", "Broward-Miami")) %>%
  filter(LAT_DEGREES < 25.97)

# Calculate average and standard error for DIS_PREV and BLE_PREV by year
summary_stats <- mdcBiscDisBle %>%
  group_by(YEAR) %>%
  summarize(
    avg_DIS_PREV = mean(DIS_PREV, na.rm = TRUE),
    se_DIS_PREV = sd(DIS_PREV, na.rm = TRUE) / sqrt(n()),
    avg_BLE_PREV = mean(BLE_PREV, na.rm = TRUE),
    se_BLE_PREV = sd(BLE_PREV, na.rm = TRUE) / sqrt(n()),
    .groups = 'drop'
  )

# Create a plot for Disease Prevalence (DIS_PREV)
plot_dis_prev <- ggplot(summary_stats, aes(x = YEAR, y = avg_DIS_PREV)) +
  geom_line(color = "yellow", size = 1) +
  geom_point(color = "yellow", size = 2) +
  geom_errorbar(aes(ymin = avg_DIS_PREV - se_DIS_PREV, ymax = avg_DIS_PREV + se_DIS_PREV), width = 0.2, color = "yellow") +
  labs(x = "Year", y = "Average Disease Prevalence") +
  ggtitle("Disease Prevalence") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))+
  coord_cartesian(ylim = c(0, NA))  # Ensure y-axis starts at 0

# Create a plot for Bleaching Prevalence (BLE_PREV)
plot_ble_prev <- ggplot(summary_stats, aes(x = YEAR, y = avg_BLE_PREV)) +
  geom_line(color = "darkred", size = 1) +
  geom_point(color = "darkred", size = 2) +
  geom_errorbar(aes(ymin = avg_BLE_PREV - se_BLE_PREV, ymax = avg_BLE_PREV + se_BLE_PREV), width = 0.2, color = "darkred") +
  labs(x = "Year", y = "Average Bleaching Prevalence") +
...
combined_plot <- plot_dis_prev | plot_ble_prev

# Save the combined plot
ggsave("./figs/combined_prevalence_plot.png", plot = combined_plot, width = 12, height = 6, units = "in")
...
```{r, density top spp.}
DRM_SCREAM_FL_1999_13_density_species
NCRMP_FLK_2014_22_density_species
NCRMP_SEFCRI_2014_22_density_species

combined_density_species <- bind_rows(
  DRM_SCREAM_FL_1999_13_density_species,
  NCRMP_FLK_2014_22_density_species,
  NCRMP_SEFCRI_2014_22_density_species
)

mdcBiscDensity <- combined_density_species %>%
  filter(SUB_REGION_NAME %in% c("Biscayne", "SEFCRI"),
         LAT_DEGREES < 25.97)

# Calculate total density for each species
species_total_density <- mdcBiscDensity %>%
  group_by(SPECIES_NAME) %>%
  summarize(
    total_density = sum(DENSITY, na.rm = TRUE),  # Replace DENSITY with the actual column name if different
    .groups = 'drop'
  )

# Identify the top 10 species with the highest total density
top_10_species <- species_total_density %>%
  arrange(desc(total_density)) %>%
  head(10)

# View the top 10 species
...
top_species <- c(
  ...
  "Porites divaricata",
  "Orbicella faveolata"  # Added this species as it was listed without a density
)

# Filter data for the top 9 species
filtered_data <- filter(mdcBiscDensity, SPECIES_NAME %in% top_species)

# Calculate average density and standard error for each year and species
summary_stats <- filtered_data %>%
  group_by(YEAR, SPECIES_NAME) %>%
  summarize(
    average_Density = mean(DENSITY, na.rm = TRUE),  # Replace DENSITY with the correct column name if needed
    se_Density = sd(DENSITY, na.rm = TRUE) / sqrt(n()),  # Standard error calculation
    .groups = 'drop'
  )

# View the summary statistics to check the result
print(summary_stats)

# Get color palette with 10 colors
colPal <- paletteer::paletteer_d("NineteenEightyR::youngturqs", n = 10)
...
for (species_name in top_species) {
  species_data <- filter(summary_stats, SPECIES_NAME == species_name)
  
  plot <- ggplot(species_data, aes(x = YEAR, y = average_Density, group = SPECIES_NAME)) +
    geom_line(color = colPal[which(top_species == species_name)], size = 1) +
    geom_point(color = colPal[which(top_species == species_name)], size = 2) +
    geom_errorbar(aes(ymin = average_Density - se_Density,
                      ymax = average_Density + se_Density),
                  width = 0.2, color = colPal[which(top_species == species_name)]) +
    labs(x = "Year",
         y = "Average Density") +
    ...
    coord_cartesian(ylim = c(0, NA))  # Ensure y-axis starts at 0
  plots[[species_name]] <- plot
}

# Combine the plots using patchwork
combined_plot <- wrap_plots(plots, ncol = 5)  # Arrange in 5 columns

# Save the final plot
ggsave("./figs/combined_density_plot.png", plot = combined_plot, width = 15, height = 10, units = "in")

```

##Change in benthic cover
```{r, Change in cover}
sefcriCoverSite=NCRMP_SEFCRI_2014_22_percent_cover_site #I think this is what I want
flkCoverSite=NCRMP_FLK_2014_22_percent_cover_site
# Combine and filter data
mdcBiscCover <- bind_rows(sefcriCoverSite, flkCoverSite) %>%
  filter(SUB_REGION_NAME %in% c("Biscayne", "Broward-Miami")) %>%
  filter(LAT_DEGREES < 25.97)

# Prepare data for all cover groups
cover_data <- mdcBiscCover %>%
  filter(cover_group %in% c("HARD CORALS", "MACROALGAE", "TURF ALGAE")) %>%
  dplyr::select(YEAR, cover_group, Percent_Cvr) %>%
  mutate(YEAR = as.factor(YEAR))  # Convert YEAR to a factor

# Calculate averages and standard errors for each cover group
average_cover <- cover_data %>%
  group_by(YEAR, cover_group) %>%
  summarise(
    average_Percent_Cvr = mean(Percent_Cvr, na.rm = TRUE),
    sd_Percent_Cvr = sd(Percent_Cvr, na.rm = TRUE),
    n = n(),
    se_Percent_Cvr = sd_Percent_Cvr / sqrt(n)  # Standard error
  )

# Perform ANOVA for each cover group
anova_results <- list()
for (group in unique(cover_data$cover_group)) {
  anova_results[[group]] <- aov(Percent_Cvr ~ YEAR, data = filter(cover_data, cover_group == group))
}

# Tukey's HSD post-hoc test
tukey_results <- list()
for (group in names(anova_results)) {
  tukey_results[[group]] <- TukeyHSD(anova_results[[group]])
  print(tukey_results[[group]])
}

# Define colors for each cover group
color_map <- c("HARD CORALS" = "blue", "MACROALGAE" = "green", "TURF ALGAE" = "brown")

# Create plots for each cover group with lines and error bars

# Ensure data is properly prepared
mdcBiscCover <- mdcBiscCover %>%
  filter(cover_group %in% c("HARD CORALS", "MACROALGAE", "TURF ALGAE")) %>%
  group_by(YEAR, cover_group) %>%
  summarise(
    average_Percent_Cvr = mean(Percent_Cvr, na.rm = TRUE),
    se_Percent_Cvr = sd(Percent_Cvr, na.rm = TRUE) / sqrt(n()),
    .groups = "drop"
  )

# Define the common y-axis limits (if you want to use the same limit for all plots)
y_axis_limits <- c(0, NA)  # NA allows the upper limit to be automatically determined

# Plot for HARD CORALS
plot_coral <- ggplot(filter(mdcBiscCover, cover_group == "HARD CORALS"), aes(x = YEAR, y = average_Percent_Cvr, group = cover_group)) +
  geom_line(color = color_map["HARD CORALS"], size = 1) +
  geom_point(color = color_map["HARD CORALS"], size = 2) +
  geom_errorbar(aes(ymin = average_Percent_Cvr - se_Percent_Cvr,
                    ymax = average_Percent_Cvr + se_Percent_Cvr),
                width = 0.2, color = color_map["HARD CORALS"]) +
  labs(x = "Year",
...
# Plot for MACROALGAE
plot_macroalgae <- ggplot(filter(mdcBiscCover, cover_group == "MACROALGAE"), aes(x = YEAR, y = average_Percent_Cvr, group = cover_group)) +
  geom_line(color = color_map["MACROALGAE"], size = 1) +
  geom_point(color = color_map["MACROALGAE"], size = 2) +
  geom_errorbar(aes(ymin = average_Percent_Cvr - se_Percent_Cvr,
                    ymax = average_Percent_Cvr + se_Percent_Cvr),
                width = 0.2, color = color_map["MACROALGAE"]) +
  labs(x = "Year",
       y = "Average Percent Cover") +
  ggtitle("Macroalgae") +
  theme_minimal() +
...
# Plot for TURF ALGAE
plot_turf_algae <- ggplot(filter(mdcBiscCover, cover_group == "TURF ALGAE"), aes(x = YEAR, y = average_Percent_Cvr, group = cover_group)) +
  geom_line(color = color_map["TURF ALGAE"], size = 1) +
  geom_point(color = color_map["TURF ALGAE"], size = 2) +
  geom_errorbar(aes(ymin = average_Percent_Cvr - se_Percent_Cvr,
                    ymax = average_Percent_Cvr + se_Percent_Cvr),
                width = 0.2, color = color_map["TURF ALGAE"]) +
  labs(x = "Year",
       y = "Average Percent Cover") +
  ggtitle("Turf Algae") +
  theme_minimal() +
...
# Combine the plots using patchwork
combined_plot <- (plot_coral | plot_macroalgae | plot_turf_algae) +
  plot_layout(guides = "collect")

# Save the final plot
ggsave("./figs/combined_cover_plot.png", plot = combined_plot, width = 16, height = 6, units = "in")


```

##Data testing
Testing number of surveys because I am getting a mismatch from the two datasets, one is missing PRIMARY_SAMPLE_UNIT 1121, survey 10/21/16, 25.6212	-80.0938. Turns out this one has mostly NA's so maybe was removed down the line?
```{r, dataTesting}
mdcBiscBenthicSummary <- mdcBiscBenthic %>%
   mutate(
    LAT_DEGREES = round(LAT_DEGREES, 4),   # Round LAT_DEGREES to 4 decimal places
    LON_DEGREES = round(LON_DEGREES, 4))%>%    # Round LON_DEGREES to 4 decimal places
  distinct(PRIMARY_SAMPLE_UNIT, YEAR, MONTH, DAY, LAT_DEGREES, LON_DEGREES)%>%
  arrange(PRIMARY_SAMPLE_UNIT) %>%
    mutate(LAT_DEGREES = as.character(LAT_DEGREES))%>%
    mutate(LON_DEGREES = as.character(LON_DEGREES))

write.csv(mdcBiscBenthicSummary, "./data/mdcBiscBenthicSummary.csv")

mdcBiscCoverSummary<- mdcBiscCover %>%
  distinct(PRIMARY_SAMPLE_UNIT, LAT_DEGREES, LON_DEGREES)%>%
  arrange(PRIMARY_SAMPLE_UNIT) %>%
    mutate(LAT_DEGREES = as.character(LAT_DEGREES))%>%
    mutate(LON_DEGREES = as.character(LON_DEGREES))

write.csv(mdcBiscCoverSummary, "./data/mdcBiscCoverSummary.csv")
