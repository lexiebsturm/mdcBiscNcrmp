...
pacman::p_load("paletteer", "patchwork", "tidyverse", "vegan",  "dplyr", "ncrmp.benthics.analysis", "ggplot2", "sf", "maps", "ggmap", "ggspatial", "tigris", "glue")
...
# Define full species names
species_names <- c(
  "ACR CERV" = "Acropora cervicornis",
  "ACR PALM" = "Acropora palmata",
  "DEN CYLI" = "Dendrogyra cylindrus",
  "MYC FERO" = "Mycetophyllia ferox",
  "ORB ANNU" = "Orbicella annularis",
  "ORB FAVE" = "Orbicella faveolata",
  "ORB FRAN" = "Orbicella franksi",
  "ORB SPE." = "Orbicella spp."
)
# Update species codes to full names in your dataset
esaSppFOI_long <- esaSppFOI %>%
  pivot_longer(cols = starts_with("prop_"),
               names_to = "Species",
               values_to = "Proportion") %>%
  mutate(Species = gsub("prop_", "", Species)) %>%
  mutate(Species = factor(Species, levels = names(species_names)),
         Species = recode(Species, !!!species_names))  # Replace species codes with full names

# Function to create plot with italicized species names in titles
create_plot <- function(species_name) {
  ggplot(esaSppFOI_long %>% filter(Species == species_name), aes(x = YEAR, y = Proportion, group = Species, color = Species)) +
    geom_line(size = 1) +
    geom_point(size = 2) +
    labs(title = glue("**{species_name}** Presence/Absence Across Surveys"),  # Use glue for dynamic titles
         x = "Year",
         y = "Proportion of Surveys") +
    scale_color_manual(values = "blue") + # Customize color if needed
...
plot_list <- lapply(species_names, function(species_name) {
  create_plot(species_name)
})
...
hardCoralPlot=ggplot(average_coral_cover_by_year, aes(x = YEAR, y = avg_hard_coral_cover, group=1)) +
  ...
  labs(
       x = "Year",
       y = "Average Percent Hard Coral Cover") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Rotate x-axis labels if needed

ggsave(filename = "./figs/hardCoralPlot.png", plot = hardCoralPlot, width = 10, height = 6, dpi = 300)
